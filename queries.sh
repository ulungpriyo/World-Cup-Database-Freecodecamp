...
echo "$($PSQL "SELECT DISTINCT t1.name AS team FROM teams AS t1 INNER JOIN games ON t1.team_id = games.winner_id OR t1.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY t1.name")"
...
echo "$($PSQL "SELECT DISTINCT t1.name FROM teams AS t1 INNER JOIN games ON t1.team_id = games.winner_id OR t1.team_id = games.opponent_id WHERE t1.name ILIKE 'co%'")"
